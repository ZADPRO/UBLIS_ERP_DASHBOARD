...
const GMeetMembersSidebar: React.FC<GMeetMembersSidebarProps> = ({
  ...
  const deleteMembers = async (email: string, meetingId: string) => {
    ...
    if (data.token == false) {
      ...
      if (data.success) {
        toast.success(
          `Student removed from the meeting Successfully`,
          {
            position: "top-right",
            autoClose: 5000,
        ...
        toast.error(
          `can not remove the student form the meeting link`,
          {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
  ...
  const handleRemoveMeeting = (Data: any, meetingId: string) => {
    toast.warning(
      <div>
        <p>Are you sure you want to remove <b>{Data.refSCustId} - {Data.refStFName} {Data.refStLName}</b>?</p>
  ...
  const [selectedMembers, setSelectedMembers] = useState<{ name: string; code: string }[]>([]);
  const [refSelectedMembers, setRefSelectedMembers] = useState<{ name: string; code: string }[]>([]);
  ...
  return (
    <div>
      <ToastContainer />
      <div className="mb-2"><span className="text-[#f95005] font-bold text-[1.2rem]">Meeting Info</span></div>
      <div className="border-2 border-gray-400 rounded-xl">
        <div className="flex flex-row justify-start">
          <div className="w-[50%] flex flex-row">
            <p className="w-[100%]"><span className="text-[#f95005] font-bold">Title : </span>{selectedMeeting?.refMeetingTitle}</p>
          </div>
          <div className="w-[50%] flex flex-row justify-between align-items-center">
            <p className="w-[80%]"><span className="text-[#f95005] font-bold">Create Date : </span>{selectedMeeting?.refCreatedDate}</p>
            <button
            ...
            >
              {meetingInfo ? <><IoIosArrowUp size={"1.5rem"} /></> : <><IoIosArrowDown size={"1.5rem"} /></>}
        ...
        {meetingInfo ?
          <>
            <div className="flex flex-row justify-start">
              <div className="w-[50%] flex flex-row">
                <p className="w-[50%]"><span className="text-[#f95005] font-bold">Start Date : </span>{selectedMeeting?.refMeetStartFrom}</p>
                <p className="w-[50%]"><span className="text-[#f95005] font-bold">End Date : </span>{selectedMeeting?.refMeetEnd}</p>
              </div>
              <div className="w-[50%] flex flex-row">
                <p className="w-[50%]"><span className="text-[#f95005] font-bold">Start Time : </span>{selectedMeeting?.refStartTime}</p>
                <p className="w-[50%]"><span className="text-[#f95005] font-bold">End Time : </span>{selectedMeeting?.refEndTime}</p>
              </div>

            </div>
            <div className="flex flex-row justify-start">
              <div className="w-[50%] flex flex-row">
                <p className="w-[50%]"><span className="text-[#f95005] font-bold">Meeting : </span>{selectedMeeting?.refMLTypeName}</p>
                <p className="w-[50%]"><span className="text-[#f95005] font-bold">Branch : </span>{selectedMeeting?.refBranchName}</p>
              </div>
              <div className="w-[50%] flex flex-row">
                <p className="w-[100%]"><span className="text-[#f95005] font-bold">Link : </span>{selectedMeeting?.refMeetingLink}</p>
                <button
                  onClick={() => {
                    if (
                      navigator.clipboard &&
            ...
            <div className="flex flex-row justify-start">
              <p className="w-[100%]"><span className="text-[#f95005] font-bold">Description : </span>{selectedMeeting?.refMeetingDescription}</p>
            </div>
          </>
          :
          <>
      ...
      <div className="mt-4 mb-2 flex flex-row justify-between align-items-center">
        <span className="text-[#f95005] font-bold text-[1.2rem]">Student List</span>
        {addMembers ?
          <>
            <button
          ...
          :
          <>
            <button className="border-none bg-[#28a745] text-white px-5 py-2 text-[1rem]" onClick={() => { setAddMembers(true) }}>Add Student</button>
          </>
        }
      </div>
              ...
              <div className="w-full flex flex-col justify-center align-items-center">
                ...
                <div className="flex w-[100%] align-items-center">
                  ...
                  <div className="flex justify-end w-[20%]">
                    <button className="border-none rounded-md bg-[#28a745] text-white px-5 py-2 text-[1rem] " onClick={AddNewMembers}>Add</button>
                  </div>
                </div>
                {searchList ? (
                  <div className="flex w-full align-items-center justify-center ">
                    ...
                    >
                      ...
                      />
                      <Column field="refCtMobile" header="Mobile" style={{ minWidth: "10rem" }} />
                      <Column field="refCtEmail" header="Email" style={{ minWidth: "8rem" }} />
                      <Column field="refStDOB" header="DOB" style={{ minWidth: "8rem" }} />
                    </DataTable>
                ...
                <div className="w-[100%] flex flex-col justify-center align-items-center">
                  <span className="font-bold text-[#f95005] text-[1rem] w-[80%]">Selected Members</span>
      ...
      <DataTable value={gmeetStudentDetails}>
        ...
        <Column field=""
          body={(rowData) => (<div className="w-[100%] flex justify-center" >
            <button
              className="border-none rounded p-2 bg-transparent text-red-500 hover:text-white hover:bg-red-500"
              onClick={() => { handleRemoveMeeting(rowData, selectedMeeting.refGoogleMeetId) }}
            >
